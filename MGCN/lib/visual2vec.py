...
def visual2vec(opt, tokenizer):
    ...
    # initialize
    data_concepts_visual2vec=torch.zeros(opt.num_attribute,2048).cuda()
    count = torch.zeros(opt.num_attribute,1).cuda() + 1e-10
    ...
    for i, train_data in enumerate(train_loader):
        ...
        attribute_labels=attribute_labels[:,2*opt.num_attribute:].cuda()
    ...
    outfile=os.path.join(opt.data_path,opt.data_name,'Concept_annotations',opt.data_name+'_concepts_visual2vec_{}.pkl'.format(str(opt.num_attribute)))
