...
class PrecompRegionDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenizer, opt, train):
        ...
        loc_cap = osp.join(data_path, data_name, 'precomp')
        loc_image = osp.join(data_path, data_name, 'precomp')
        ...
        with open(categroy_concepts_dir, "rb") as names_concepts:
            ...
            self.text_concepts = concepts['text_concepts']
            self.visual_objects = concepts['visual_objects']

        self.text_classes = len(self.text_concepts)
        self.visual_classes = len(self.visual_objects)
    ...
    def __getitem__(self, index):
        ...
        ''' generate the concept labels for each sentence'''
        concept_labels = np.zeros(self.text_classes + self.visual_classes, np.float32)
        for (i, word) in enumerate(caption_tokens):
            try:
                ...
                if tag.startswith('NN'):
                    ...
                    word_lemma = lemmatizer.lemmatize(word, pos='r')
                else:
                    word_lemma = word
            except:
                continue
            if word_lemma in self.text_concepts:
                inx_concept = self.text_concepts.index(word_lemma)
                concept_labels[inx_concept] = 1

        for i, visual_object in enumerate(self.visual_objects):
            if visual_object in caption:
                concept_labels[i + self.text_classes] = 1

        concept_labels = torch.Tensor(concept_labels)
        return image, target, concept_labels, index, img_index
...
def collate_fn(data):
    ...
    """
    images, captions, attribute_labels, ids, img_ids = zip(*data)
    if len(images[0].shape) == 2:  # region feature
        ...
        attribute_labels = torch.stack(attribute_labels, 0)

        return all_images, img_lengths, targets, lengths, attribute_labels, ids
...
'''
def get_loaders(data_path, data_name, attribute_path, tokenizer, batch_size, workers, opt):
    train_loader = get_loader(data_path, data_name, attribute_path,'train', tokenizer, opt,
                              batch_size, True, workers)
    val_loader = get_loader(data_path, data_name, attribute_path,'dev', tokenizer, opt,
                            batch_size, False, workers, train=False)
    return train_loader, val_loader


def get_train_loader(data_path, data_name, attribute_path, tokenizer, batch_size, workers, opt, shuffle):
    train_loader = get_loader(data_path, data_name, attribute_path, 'train', tokenizer, opt,
                              batch_size, shuffle, workers)
    return train_loader

def get_test_loader(split_name, data_name, attribute_path, tokenizer, batch_size, workers, opt):
    test_loader = get_loader(opt.data_path, data_name, attribute_path, split_name, tokenizer, opt,
                             batch_size, False, workers, train=False)
    return test_loader
'''
