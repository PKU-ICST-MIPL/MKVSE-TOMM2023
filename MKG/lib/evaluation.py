...
def encode_data(model, data_loader, log_step=10, logging=logger.info, backbone=False):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if not backbone:
            images, image_lengths, captions, lengths, attribute_labels, ids = data_i
        else:
        ...
        if not backbone:
            img_emb, cap_emb, _, _ = model.forward_emb(images, image_lengths, captions, lengths, attribute_labels)
        else:
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None, cxc=False):
    ...
    # It has been moved to eval.py and changed to the right path.
    # opt.backbone_path = '../data/vse_infty/weights/original_updown_backbone.pth'
    ...
    data_loader = image_caption.get_loader(opt.data_path, opt.data_name,
                                           split, tokenizer, opt, opt.batch_size, False,
                                                opt.workers, train=False)
