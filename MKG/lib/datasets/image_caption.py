...
class PrecompRegionDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenizer, opt, train):
        ...
        categroy_concepts_dir = os.path.join(opt.data_path, opt.data_name,
                                           'Concept_annotations','category_concepts.json')
        with open(categroy_concepts_dir, "r") as names_concepts:
            concepts2ind = json.load(names_concepts)
        self.name_concepts = []
        for i, (k, v) in enumerate(concepts2ind.items()):
            k = lemmatizer.lemmatize(k)  # get the lemma form of words
            self.name_concepts.append(k)

        self.num_classes = len(self.name_concepts)
    ...
    def __getitem__(self, index):
        ...
        concept_labels = np.zeros(self.num_classes, np.float32)
